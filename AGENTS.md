...
description: Entry point and activation contract for Git Platform Enhancer — resolves the shared set, then routes to this repository's local instructions.
---

# Git Platform Enhancer — Agent Entry Point

Git Platform Enhancer is a Manifest V3 browser extension that injects a floating utility
menu into GitHub and GitLab pages, providing context-aware navigation, VS Code clone and
Dev Container hand-off, and a favorites manager backed by `chrome.storage.local`. It is
plain browser JavaScript with no build step, no dependencies, and no test framework.
...
[`{shared}/rules/auto-activation.md`](agents://rules/auto-activation.md); the shared rows
below mirror it row-for-row. Local rows are appended beneath them.
...
| Edit `manifest.json`, a content script, or an extension permission | [`.agents/rules/browser-extension.md`](.agents/rules/browser-extension.md) |
| Touch favorites storage, its record shape, or a data migration | [`.agents/wiki/domain/favorites-data-safety.md`](.agents/wiki/domain/favorites-data-safety.md) |

## Reading Order

`AGENTS.md` → resolve the shared set → [`.agents/index/root-index.md`](.agents/index/root-index.md)
and nothing else at this stage → the ONE index whose scope matches → one child branch if it
delegates → only then the specific files.
...
* `AGENTS.md` and `README.md` are overviews and must never carry detailed rules or
  documentation.
* `.agents/index/root-index.md` is a **router only**. It lists other indexes. It must never
...
* Memory → `.agents/memory/{type}/{file-name}.md`; indexes → `.agents/index/{scope}-index.md`;
  anything universal goes to the shared set. No `INDEX.md`, anywhere, ever.

## Discovery Protocol

Source of truth:
[`{shared}/rules/discovery-protocol.md`](agents://rules/discovery-protocol.md).
...
Never change the project version without explicit user approval. The version carrier in
this repository is the `version` field in `manifest.json`, mirrored by the release
directory under `wiki/logs/{Major}/{Minor}/{Patch}/`. See
[`{shared}/rules/versioning.md`](agents://rules/versioning.md).
...
[`{shared}/rules/no-session-links.md`](agents://rules/no-session-links.md).
